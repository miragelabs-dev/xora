...
import { alias, BuildAliasTable, pgView, QueryBuilder } from "drizzle-orm/pg-core";
...
const buildReplyToJson = (
  ...
  nftsTable: BuildAliasTable<typeof nfts, 'reply_to_nft'>
) =>
  sql<
    {
      id: number;
...
export const postView = pgView('post_view').as((qb) => {
  ...
  const replyToNftsAlias = alias(nfts, 'reply_to_nft');
  ...
  return qb
    .select({
      ...
      nft: buildNftJson(sql`${posts.id}`).as('nft'),
    })
    .from(posts)
    ...
    .groupBy(
      ...
      sql`${alias(users, 'reply_to_author').isCryptoBot}`,
      sql`${alias(users, 'reply_to_author').isVerified}`,
    );
});
